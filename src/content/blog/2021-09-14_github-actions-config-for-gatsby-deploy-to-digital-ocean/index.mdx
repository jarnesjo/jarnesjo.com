...
description: 'Here I share my GitHub Action to build and deploy a Gatsby JS site to Digital Ocean droplet'
date: '2021-09-14'
slug: 'github-actions-config-for-gatsby-deploy-to-digital-ocean'
category: code
tags:
  - github-actions
  - digital-ocean
...
And ok it mostly for my own sake because I forget everything and want a cheatsheet to go back to, but hope it can help
anybody because I couldn't find it when Googling.
...
`DEPLOY_KEY` holds the private SSH key so best way is to create a new pair for just this actions. You do so in terminal
with `ssh-keygen` and put it in some temporary directory and naming. **Do not** use the default naming and place it that
case you may overwrite existing key. The private key you put into the Github secrets and the private key you add to your
Digital Ocean droplet into `~/.ssh/authorized_keys`. You can do so by copy and paste by editing the file with
